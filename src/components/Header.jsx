...
export default function Header() {
  ...
  const { isActive, status, location: speechLocation, supported: speechSupported } = useSpeechReader();
  ...
  const { todayCompleted, panelOpen, openPanel, closePanel } =
    useReadingStamina();
  ...
  return (
    <header className="header">
      <div className="header-brand">
        <Link to="/" className="header-logo" aria-label={appConfig.name}>
          {isImageIcon(appConfig.icon) ? (
            <img
              src={assetUrl(appConfig.icon)}
              alt=""
              className="header-logo-icon"
            />
          ) : (
            <span className="header-logo-emoji" aria-hidden>
              {appConfig.icon}
            </span>
          )}
        ...
        >
          <LightningIcon size={22} className="stamina-trigger-icon" />
        </button>
      </div>
      ...
      <div className="header-version">
        <div className="header-actions">
          ...
          >
            ...
            {needRefresh && (
              <span className="header-actions-update-dot" aria-hidden />
            )}
          </button>
      ...
      {readingSettingsOpen && (
        <ReadingSettingsPanel onClose={() => setReadingSettingsOpen(false)} />
      )}
